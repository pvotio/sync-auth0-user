...
class App:
    ...
    IGNROE_USERS = settings.CLIENT_IGNORE_USERS
    ...
    def __init__(self):
        ...
        """
        self.db = helper.init_db_instance()
    ...
    def run(self):
        ...
        """
        logger.info(f"Found {len(self.db_users)} users in the database.")
        logger.info(f"Found {len(self.users)} users in Auth0.")
        logger.info(f"Found {len(self.roles)} roles in Auth0.")
        logger.info(f"Found {len(self.IGNROE_USERS)} users in ignore list")
    ...
    def sync_update_user_role(self):
        ...
        logger.info("Syncing updated user roles done.")
